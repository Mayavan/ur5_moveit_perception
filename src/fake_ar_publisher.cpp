...
static void pubVisualMarker(const ur5_moveit_perception::ARMarker& m) {
...
void pubCallback(const ros::TimerEvent&) {
  ...
  ur5_moveit_perception::ARMarker m;
...
bool getTarget(ur5_moveit_perception::getTarget::Request  &req,
         ur5_moveit_perception::getTarget::Response &res)
{
  geometry_msgs::Pose p = pose();
  ur5_moveit_perception::ARMarker m;
  m.header.frame_id = camera_frame_name();
...
int main(int argc, char** argv) {
  ...
  ar_pub = nh.advertise<ur5_moveit_perception::ARMarker>("ar_pose_marker", 1);
  visual_pub = nh.advertise<visualization_msgs::Marker>("ar_pose_visual", 1);

  // init pose
  pose().orientation.w = 1.0;  // facing straight up
  pnh.param<double>("x_pos", pose().position.x, 0.4);
  pnh.param<double>("y_pos", pose().position.y, 0.1);
  ...
  // Advertise service
  ROS_INFO("Starting the service to send the pose");
  ros::ServiceServer service = nh.advertiseService("ar_pose_service", getTarget);
